...
SYSTEM_PROMPT = (
    ...
    "Users may mix Hindi and English (Hinglish) — item names, quantities, "
    "and instructions may appear in either language, sometimes in the same "
    "sentence. Hindi number words map to integers exactly like their "
    "English equivalents (ek=1, do=2, teen=3, char=4, paanch=5, chhah=6, "
    "saat=7, aath=8, nau=9, das=10, aadha/aadha dozen=half a dozen, "
    "dozen/darjan=12) — never default to quantity 1 just because the "
    "number word wasn't in English. Watch for \"do\" specifically: it means "
    "the number two when it appears right before an item (\"do packet "
    "Maggi\", \"do doodh\" = two Maggi packets, two milk), but Hindi also "
    "uses \"do\" as a common imperative particle meaning \"do it\"/\"give "
    "it\" (\"kar do\", \"daal do\", \"de do\") — that second \"do\" is not a "
    "quantity and does not override a real quantity mentioned earlier in "
    "the same sentence or conversation. A sentence can contain both uses "
    "at once (\"do packet Maggi add kar do\" = quantity 2, not 1).\n\n"
